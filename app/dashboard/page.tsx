...
export default function EntryGatePage() {
  const { slots, setSlots, setActiveVehicles } = useParking();
    ...
    <main className="h-screen w-full bg-slate-950 text-slate-100 flex flex-col font-sans overflow-hidden">
      ...
      <section className="flex-1 flex flex-col items-center justify-center p-8 relative">
        <div className="text-center mb-16 relative z-10">
          ...
          <p className="text-lg text-slate-400">Tekan tombol di bawah untuk membuka gerbang</p>
        </div>

        {/* Big Red Button */}
        <div className="relative group">
          {/* Outer glowing rings */}
          <div className={`absolute -inset-8 bg-rose-600/20 rounded-full blur-2xl transition-all duration-500 ${availableSlots > 0 ? 'group-hover:bg-rose-500/40 group-hover:blur-3xl' : 'opacity-0'}`}></div>
          <div className={`absolute -inset-4 bg-rose-500/30 rounded-full blur-xl transition-all duration-500 ${availableSlots > 0 ? 'group-hover:bg-rose-400/50' : 'opacity-0'}`}></div>
          
          <button
            onClick={handleAmbilTiket}
            disabled={availableSlots === 0 || modalData !== null}
            className={`
              relative z-10 w-64 h-64 md:w-80 md:h-80 rounded-full flex flex-col items-center justify-center 
              border-8 border-slate-900 shadow-[inset_0_4px_20px_rgba(255,255,255,0.2),_0_10px_30px_rgba(0,0,0,0.8)]
              transition-all duration-200 
              ${availableSlots > 0 && !modalData
                ? 'bg-gradient-to-br from-rose-500 to-rose-700 hover:from-rose-400 hover:to-rose-600 active:scale-95 active:shadow-[inset_0_10px_30px_rgba(0,0,0,0.5)] cursor-pointer' 
                : 'bg-slate-800 border-slate-900 text-slate-600 cursor-not-allowed'
              }
            `}
          >
            <span className={`text-2xl md:text-3xl font-black uppercase tracking-widest text-center px-6 leading-relaxed 
              ${availableSlots > 0 && !modalData ? 'text-white' : 'text-slate-500'}
            `}>
              {availableSlots === 0 ? 'Parkir Penuh' : 'TEKAN UNTUK AMBIL TIKET'}
            </span>
            {availableSlots > 0 && !modalData && (
